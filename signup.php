...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insert_query = "INSERT INTO users (id ,username, PhoneNumber,password) VALUES (?,?,?,?)";
    $insert_stmt = $conn->prepare($insert_query);
    $insert_stmt->bind_param("ssss", $id,$username, $PhoneNumber,$hashed_password);

    if ($insert_stmt->execute()) {
        // Signup successful, redirect to the login page with a success message
        header("Location: index1.html?signup=success");
        exit();
    } else {
